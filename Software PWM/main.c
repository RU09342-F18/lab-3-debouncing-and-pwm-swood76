...
int PWM = 500; // Initialized PWM as global constant of 500, so that on board LED duty cycle of 50 percent is achieved intially
...
{
    //button configure
        P1DIR &= ~BIT3; // Initializes Button 3 (P1.3) as an input
        ...
        P1IES |= BIT3; // Configures interrupt to occur when the button transition from a state of high to low, so when the TOGGLE INTERRUPT EDGE: HIGH TO LOW
...
__interrupt void Timer_A0 (void)
{
            ...
            if(ButtonEdge == 0)
         {
                ...
                P1IE |= BIT3; // Re-Configures interrupt to be high on the P1IE register so when button is pressed; the PORT1 ISR is enabled
                ...
                {
                        P1IE |= BIT3;  // Re-Configures interrupt to be high on the P1IE register so when button is pressed; the PORT1 ISR is enabled
                        P1IES |= BIT3;    // Configure Interrupt to occur on the high to low state transition since button has been pressed
                        P1IFG &= ~BIT3; // interrupt has not occurred, flag is cleared
                        TA1CTL &= ~TASSEL_2; // TimerA1's Control register set to disable the submaster clock
                        TA1CTL |= TACLR;    // TimerA1"s control register is cleared and reset
...
__interrupt void PORT_1(void)
{
        ...
        if(ButtonEdge == 0)
        {
            ...
            P1IFG &= ~BIT3; // To verify button has not been pressed and interrupt has not occurred, flag is cleared
        ...
        else if(ButtonEdge == 1) //ON -> GOING OFF
        {
            TA1CTL = TASSEL_2 + MC_1 + TACLR; // control of timerA1 is selected to enable TimerAs clock source to subMaster Clock
                                              // and that the desired value is counted to by selecting Up Mode since
            P1IFG &= ~BIT3; // To verify button has been pressed and interrupt has occurred, flag is cleared
